...
// Import will be done dynamically to avoid edge runtime issues
  ...
  callbacks: {
    async jwt({ token, account }): Promise<XeroJWT> {
      ...
      try {
        ...
        const { withDistributedLockRetry } = await import('./redis/redisClient');
        ...
        // If we can't get the lock or import fails, refresh without lock
